...
from HoughCircleParams import exp_to_params
...
def mean_colour(img_bgr, exp):
    ...
    params = exp_to_params(exp)

    blurred = cv.GaussianBlur(gray, (params[4], params[4]), 0)
    # blurred = gray
    # cv.imshow('i', blurred)
    # cv.waitKey(0)
    # cv.destroyAllWindows()
    ...
    circle = cv.HoughCircles(blurred, cv.HOUGH_GRADIENT, params[0], params[1], param1=params[2], param2=params[3],
    ...
    if circle is not None:
        ...
        if len(circle[0]) != 1:
            ...
            cv.circle(circle_mask, (row, col), rad, 1, thickness=-1)
...
def dist(col1, col2, col_scale):
    if col_scale == C.BGR:
        return dist_bgr(col1, col2)
    elif col_scale == C.HSV:
        return dist_hsv(col1, col2)
    elif col_scale == C.LAB:
        return dist_lab(col1, col2)
    elif col_scale == C.YUV:
        return dist_yuv(col1, col2)
